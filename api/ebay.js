...
if (!ID || !SECRET) return res.status(500).json({ error: ‘Missing credentials’ });
...
if (!token.access_token) return res.status(500).json({ error: ‘Token failed’, detail: token });

const q = (req.query.q || ‘laptop’).replace(/ /g, ‘+’);
...
return res.status(200).json({ success: true, total: data.total || 0, products: products });
};